...
//절댓값 구하기 : Math.abs(data)170]

let gap = Math.abs(weight -normal);
console.log(height, weight, normal, gap);
// 3. 조건에 따라 결과(초과/미달/정상)를 구한다.
// document.write(`신장 : ${height}<br>체중 :${weight}<br>적정체중 :(${height}-${weight})X0.9=${normal}(kg)  <br>`);
// if(normal > gap){
//     document.write(`적정 체중은${normal}이며, 현재 적정 체중에서 ${gap}kg 초과한 것을 알 수 있습니다.`);
// }
// else{
//     document.write(`적정체중이거나 체중 미달입니다.`);
// }
let result ='';
if(weight>normal){
    result='초과한'
